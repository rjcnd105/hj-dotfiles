---
name: session-logger
description: Saves conversation history to session log files. Use when user says "세션 저장", "대화 저장", "대화 기록", "save session", or "save conversation". Automatically creates timestamped session log in sessions/ directory.
...
```
"세션 저장, 중점: 1) skill-router 생성, 2) front matter 수정"
```
...
| Input | Filename |
|-------|----------|
| "세션 저장" | `YYYY-MM-DD-session.md` |
| "세션 저장, 주제는 prd" | `YYYY-MM-DD-prd.md` |
| "오늘 대화 저장" | `YYYY-MM-DD-discussion.md` |
...
| You say | Skill does |
|---------|------------|
| "세션 저장" | Creates session log with today's date |
| "오늘 대화 저장" | Creates session log |
| "대화 기록" | Creates session log |
| "대화 저장" | Creates session log |
