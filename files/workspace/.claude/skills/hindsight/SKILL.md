...
`HINDSIGHT_BANK_ID`, `HINDSIGHT_BANK_USER`, `HINDSIGHT_API_URL` 환경변수가 모두 필요하다.
...
현재 세션을 분석하여 미래 세션의 자기 개선에 유용한 경험을 retain한다.

1. 세션을 돌아보며 아래 네 가지 관점으로 분류:
   - **성공한 접근**: 무엇이 효과적이었고, 왜 잘 되었는가
   - **실패한 접근**: 무엇이 안 되었고, 왜 실패했는가, 대안은 무엇이었는가
   - **사용자 피드백**: 사용자가 수정을 요청하거나 불만을 표현한 부분, 만족을 표현한 부분
   - **채택된 결정**: 여러 선택지 중 어떤 것을 선택했고, 그 이유는 무엇인가

2. 위 분석을 **요약하지 않고 구체적으로** 작성한다. 파일 경로, 명령어, 에러 메시지, 사용자의 실제 발언 등을 포함. Hindsight가 이 원본에서 fact을 추출한다.
...
4. REST API로 retain:
   ...
   curl -s -X POST "$HINDSIGHT_API_URL/v1/default/banks/$HINDSIGHT_BANK_ID/memories" \
     ...
     -d '{
       "items": [{
         "content": "<세션 경험 전체>",
         "context": "claude-code session experience log: <세션 주제 요약>. includes successes, failures, user feedback, and decisions.",
...
1. 현재 bank 설정 확인:
   ```bash
   curl -s "$HINDSIGHT_API_URL/v1/default/banks/$HINDSIGHT_BANK_ID" | python3 -m json.tool
   ```
