...
- INDEX.md는 generated artifact (캐시)이자 publish되는 navigation page. 손상 시 rebuild-index로 복구하되 `publish: true`를 유지하고, `publish: false` 페이지는 공개 INDEX 목록에 포함하지 않는다.
- KB 파일 변경 뒤 qmd 검색 반영은 `qmd update && qmd embed` 순서가 필요하다. `qmd embed`만으로는 새/수정 파일 discovery가 누락될 수 있다. qmd 갱신은 페이지 수정, generated index/sources, LOG 변경이 모두 끝난 뒤 실행한다.
