---
name: but
description: "Manage commits, branches, diffs, and pull requests in an existing GitButler workspace, or when the user explicitly requests GitButler. Follow the project's VCS_KIND routing; ordinary Git and Jujutsu work does not trigger this skill."
metadata:
  version: "0.22.0"
...
Use GitButler CLI (`but`) within an existing GitButler workspace. Check the
project's VCS choice before applying the commands below. `VCS_KIND=jj` routes
to the Jujutsu workflow. For a Git repository, use Git unless project settings,
existing workspace state, or the user's request identify GitButler as its
owner. If `VCS_KIND` is absent, resolve it using the project's instructions.

Installing this skill does not select GitButler for a repository. Creating a
GitButler workspace with `but setup` requires a request to adopt GitButler.
