...
| **Sub-phase A / B / C** | 이 플랜 내부 실행 단계 (인프라 선언 / 컷오버 / 은퇴) | 이 플랜의 Implementation Units 구성 |

아래 "Phase 1" 등 숫자 표기는 **로드맵 Phase**를 가리키고, "Phase A/B/C" 또는 "Sub-phase"는 **이 플랜 내부**를 가리킨다.
...
- R7. 롤백 경로: VPS 은퇴 전까지 DNS만 되돌리면 즉시 VPS로 복귀 가능
...
- **VPS 은퇴 유예**: DNS 전환 성공 후에도 VPS를 **최소 7일** 유지 (긴급 롤백 대비). `hsdb-*.tar.gz` 백업도 동일 기간 보존
...
> *이 섹션은 플랜 리뷰어가 구조를 검증하기 위한 directional guidance이며 implementation specification이 아니다. 구현자는 이를 맥락으로만 활용하고 코드로 복사하지 말 것.*
...
sequenceDiagram
    ...
    Note over Dev,VPS: 7일 유예: VPS 가동 유지 (긴급 롤백 대비)
    Dev->>VPS: 7일 후 docker compose down -v + 호스팅 종료
```
...
**Approach:**
0. **Pre-flight 실측 (Review P2-5)**: VPS에서 `sudo du -sh /var/lib/docker/volumes/hindsight-db-data` 실측. 크기 + VPS↔homelab 전송 대역폭으로 실제 downtime 추정치 재계산. "20-30분" 숫자는 이 실측 전까지 placeholder
...
- [ ] **Unit 7: 7일 유예 후 VPS 은퇴**

**Goal:** DNS 전환이 안정화되고 롤백이 불필요함을 확인 후 VPS 자원 해제.

**Requirements:** R1, R7 (롤백 유예 종료)

**Dependencies:** Unit 6 완료 + 7일 이상 안정 구동

**Files:**
- External: VPS 제공자 콘솔에서 인스턴스 종료 / 호스팅 해지
- External: `/Users/hj/study_ex/my-backend/vps` 저장소에서 `sync/services/docker-compose.yml` 정리 (arcane/caddy/hindsight 블록 제거 또는 readme에 "deprecated, migrated to homelab" 기재) — 이 플랜 범위 외(다른 저장소)

**Approach:**
1. 7일 간 Unit 4·5·6의 verification 지표 주기 확인 (외부 접속 정상, 에러율 변동 없음, 데이터 정합성 유지)
2. VPS에서 `docker compose down -v` (볼륨 포함 삭제)
3. VPS 호스팅 서비스 종료
4. `hsdb-*.tar.gz` 백업 보존 기간 결정 (최소 1개월 추천)
5. (별도 저장소 작업) VPS 저장소 docker-compose.yml·Caddyfile·mise 배포 스크립트 정리 커밋

**Execution note:** 이 unit은 주로 시간 대기와 외부 작업이므로 nix-dots 저장소의 변경은 없음. 체크박스는 "관찰·확인 완료 + VPS 서비스 종료 완료" 시점에 체크

**Test scenarios:**
- Happy path: 7일간 `hindsight.deopjib.site` 가용성 ≥ 99%
- Happy path: VPS 종료 후 `hindsight.deopjib.site` 여전히 정상 응답 (homelab 의존만 남음)
- Edge case: VPS 종료 후 첫 24시간 homelab 이상 없는지 재확인
- Error path: VPS 종료 후 예상치 못한 의존성 발견 시 `hsdb-*.tar.gz` 백업으로 새 VPS 재구축 가능

**Verification:**
- VPS 콘솔에서 인스턴스 "terminated"
- 월 청구 내역에서 VPS 요금 정지 확인
- `docs/ideation/2026-04-14-vps-to-homelab-migration-ideation.md` status를 `completed`로 업데이트
...
|---|---|---|---|
| homelab 전원/네트워크 장애로 서비스 중단 | Low | High (DNS 전환 후엔 단독 의존) | Unit 7의 7일 유예 중 가용성 모니터링. 장기 외출 시 은퇴 연기 |
...
| DNS 전파 중 일부 지역 VPS 유지 | Low | Low | TTL 60s 사전 단축. VPS 7일 유예 중이라 실제 단절 없음 |
...
| Phase C 이후 긴급 롤백 필요 (VPS 이미 종료) | Very Low | Critical | 7일 유예 기간 엄수. `hsdb-*.tar.gz` 1개월 이상 보존. 새 VPS 재구축 시 이 플랜의 Unit 3·5를 reverse 수행 |
...
체크포인트: 7일 가용성 ≥ 99% + VPS 콘솔 인스턴스 terminated.
...
- **렌즈**: feasibility
- **status**: `will-revise` — Unit 3 구현 전 확인 게이트로 추가
...
- **렌즈**: adversarial
- **status**: `will-revise` — Phase A/B 동안 "배포 전용 브랜치(`deploy`)로 comin poll 전환" 옵션을 Operational Notes에 추가, Unit 1에 `nixos-rebuild dry-activate` 선행 권장 문구 추가

**P2-3: 7일 grace period 근거 부재 + 관찰 지표 미정의**
- **발견**: "99% 가용성"의 측정 방법, RAG 품질 평가 방법, cold path sample coverage 미정의
- **렌즈**: adversarial
- **status**: `will-revise` — Unit 7을 "기간 조건"에서 "관찰 조건 만족"으로 재정의 (서로 다른 주제 5+건 retrieval 성공 + reranker 수동 품질 평가 1회 + 전력/네트워크 장애 1회 자연 발생 복구 관찰). 최소 기간 7일은 유지하되 AND 조건화

**P2-4: DNS cutover 중 클라이언트 retry 시맨틱 미검증**
- **발견**: hindsight 브라우저 에이전트가 write 실패 시 drop이면 silent data loss
- **렌즈**: adversarial
- **status**: `will-revise` — Unit 5와 Unit 6 사이 "클라이언트 retry/backoff 동작 확인" 게이트를 Unit 5.5로 추가

**P2-5: 20-30분 downtime 추정 근거 부재**
- **발견**: 데이터 크기·대역폭 실측 전 추정값
- **렌즈**: feasibility
- **status**: `will-revise` — Unit 5 Step 0에 VPS `du -sh /var/lib/docker/volumes/hindsight-db-data` 실측 선행 명시

**P2-6: TimescaleDB tar 일관성 확인 절차 부재**
- **발견**: `docker compose down` 후 종료 완료 로그 확인 단계 부재
- **렌즈**: feasibility
- **status**: `will-revise` — Unit 5 Step 3에 `docker logs <db> | grep "database system is shut down"` 확인 명시
...
- **렌즈**: coherence
- **status**: `will-revise` — 본문 일괄 교정 (이는 (a) 단계에서 함께 처리)

### 요약

- **P0 will-revise**: 3건 (P0-2, P0-3, P0-4)
- **P0 out-of-plan**: 1건 (P0-1: git history 정리 별도 분기)
- **P1 will-revise**: 3건 (P1-1, P1-3, P1-4)
- **P1 accept-with-mitigation**: 1건 (P1-2)
- **P2 will-revise**: 6건 (P2-2~6, P2-8)
- **P2 accept-with-mitigation**: 2건 (P2-1, P2-7)

**총 `will-revise` 12건** — 다음 단계 (a)에서 본문에 반영.
