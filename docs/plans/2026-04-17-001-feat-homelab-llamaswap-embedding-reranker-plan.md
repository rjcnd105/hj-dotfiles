...
- **rerank는 프록시 경유 안 함** — Cohere ↔ llama.cpp 직접 호환. 불필요한 계층 추가 없음.
...
- [ ] **Unit 1: 로컬 스모크 테스트 (macOS workspace)**
...
- [ ] **Unit 2: FastAPI embedding prefix proxy 작성**
...
- [ ] **Unit 3: ai-stack.nix — llama-swap + 프록시 NixOS 모듈 작성**
...
- [ ] **Unit 4: hindsight-stack.nix 수정 — TEI 제거 + env 변경**
...
- [ ] **Unit 5: systems/homelab/default.nix — ai-stack 통합**
...
- 적용 후 homelab에서 순차 검증:
  ...
  3. `curl http://127.0.0.1:8090/rerank -d '{"model":"qwen3-reranker","query":"cat","documents":["a cat on the mat","the weather is nice"]}'` → score 의미있는 값
...
- **Interaction graph:** Hindsight API → embed-prefix-proxy → llama-swap → llama-server(harrier | qwen3-reranker). rerank는 프록시 우회.
